...
use ItzBulkDev\Provider\Provider;
class SignManager{
    /** Supported types */
    ...
    public function __construct(Auto1v1 $plugin){
        $this->plugin = $plugin;
        ...
        foreach($this->provider->getAll() as $info){
            ...
            switch((int) $info['data']['type']){
                case SignManager::SIGN_WORLD_TELEPORT:
                    ...
                    if($show_suggest){
                        $tile->setText("[1v1]", "----------", $this->getLang("update.sign.portal.suggest"));
                        break;
                    }
                    $tile->setText(
                    ...
                    switch($info['data']['data']['id']){
                        ...
                        case 1://departure
                            if(!$this->getProvider()->exists($info['data']['data']['target']['x'], $info['data']['data']['target']['y'], $info['data']['data']['target']['z'], $info['data']['data']['target']['level'])){
                                $tile->setText("[1v1]", "----------", $this->getLang("update.sign.no-arrive"));
                                break;
                            }
                            if($info['position']['level'] === $info['data']['data']['target']['level']){
                                if($show_suggest){
                                    $tile->setText("[Auto1v1]", "----------", $this->getLang("update.sign.portal.suggest"));
                                    break;
                                }
                                $tile->setText(
                ...
                case SignManager::SIGN_COMMAND:
                    if($show_suggest){
                        $tile->setText("[1v1]", "----------", $this->getLang("update.sign.command.suggest"));
                        break;
                    }
                    $tile->setText(
